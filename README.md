...
│   ├── verify_ttt.py             # TTT 单帧循环 + 二阶 backward 验证 (Step 5A)
│   ├── verify_5b.py              # TBPTT 多段截断 + checkpoint 验证 (Step 5B)
│   └── train.py                  # TBPTT 训练循环 (Step 5B/6)
├── models/                       # 冻结权重 (~5GB)
...
预测 Δz = z_{t+1} - z_t（不是绝对 z，避免平凡解）。两个独立子头（SigLIP 196×768 + DINOv2 257×768，latent 空间不同）。

不把 intent → patch 空间当一步跨，而是 per-patch seed + 共享 MLP：

```
intent [2048] → Linear(2048, P×256) → reshape [P, 256]  ← per-patch 种子 (各 patch 一描述码)
                                    → PatchMLP(256→1024→768) → [P, 768]  ← 共享渲染器 (所有 patch 复用)
```

每个 patch 只多 256 个参数（种子维度），而非 2048×768=1.6M。变化区加权：`weight = 1 + |true_Δz|`，强制模型关注动的区域。仅全屏路（焦点路内容随 gaze 跳，无因果意义）。
...
慢权重总计 **~300M** 参数 (ElioModel 33.6M + TTTMemory 2.2M + FrameHead ~210M + AR heads ~28M + audio/gaze heads ~26M)。
...
# Step 5A: TTT 单帧记忆循环 + 二阶 backward
python -m trainer.verify_ttt --batch-size 1 --K 4

# Step 5B: TBPTT 多段截断 + checkpoint
python -m trainer.verify_5b --max-sessions 1 --K 4 --max-frames 8
```
...
### Step 5B 检查项 (K=4, checkpoint=True, 2 段 unroll)

| 检查项 | 期望 | 结果 |
|--------|------|------|
| session 多段从头滚到尾不报错 | ✅ | 2 段, K=4, 3 步预测/段 |
| checkpoint 下二阶仍通 (k_proj/v_proj grad≠0) | ✅ | 12/12 TTT params with grad |
| W 跨段数值传递 | ✅ | diff > 0 |
| 段间 W `grad_fn is None` (TBPTT 截断) | ✅ | 段间 detach 生效 |
| 显存 O(1) (peak 不随段数增长) | ✅ | ~11.2 GB/seg flat |
| Llama grad = None | ✅ | 0/146 |

> **注意**: 完整训练需 4bit 量化 (bitsandbytes) 或 >16GB GPU。FrameHead (~720M) + AdamW 状态在 bf16 下约需 14GB。
...
| Step 5A | TTT 内层循环 + 二阶 backward | ✅ 完成 |
| Step 5B | TBPTT 多段截断 + checkpoint + train.py | ✅ 完成 |
| Step 6 | 完整训练 (需 4bit + 调参) | 待实现 |
| Step 7 | 本地部署闭环 | 待实现 |
